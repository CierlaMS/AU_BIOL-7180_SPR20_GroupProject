...
#   plot-vcfstats Sal_Ref_genome.sorted.merged.g.bcfstats -p ./plots/
#
# Edit as necessary and recreate the plots by running
#   python ./plots/plot.py
#
# Title abbreviations:
# 	 0 .. Sal_R .. Sal_Ref_genome.sorted.merged.g.vcf.gz
#
...
dat = {}
with open('./plots/counts_by_af.snps.dat', 'rb') as f:
...
if plot_SNP_count_by_af:
	...
	if 0 in dat and len(dat[0])>2:
		ax1.plot([row[0]/100. for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='Sal_R')
		has_data = 1
	...
	if has_data:
		...
		plt.savefig('./plots/counts_by_af.snps.png')
		if pdf_plots: plt.savefig('./plots/counts_by_af.snps.pdf')
		plt.close()
...
dat = {}
with open('./plots/counts_by_af.indels.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
		if row[0][0] == '#': continue
		id = int(row[0])
...
if plot_Indel_count_by_af:
	...
	if has_data:
		...
		plt.subplots_adjust(bottom=0.2,left=0.1,right=0.95)
		plt.savefig('./plots/counts_by_af.indels.png')
		if pdf_plots: plt.savefig('./plots/counts_by_af.indels.pdf')
		plt.close()
...
dat = []
with open('./plots/tstv_by_af.0.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
...
if plot_tstv_by_af and len(dat)>2:
	...
	plt.title('Sal_R')
	plt.savefig('./plots/tstv_by_af.0.png')
	if pdf_plots: plt.savefig('./plots/tstv_by_af.0.pdf')
	plt.close()



dat = []
with open('./plots/tstv_by_qual.0.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
		if row[0][0] != '#': dat.append([float(x) for x in row])
...
if plot_tstv_by_qual:
	...
	plt.title('Sal_R')
	plt.savefig('./plots/tstv_by_qual.0.png')
	if pdf_plots: plt.savefig('./plots/tstv_by_qual.0.pdf')
	plt.close()



dat = []
with open('./plots/indels.0.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
		if row[0][0] != '#': dat.append([float(x) for x in row])
...
if plot_indel_dist and len(dat)>0:
	...
	plt.title('Sal_R')
	plt.savefig('./plots/indels.0.png')
	if pdf_plots: plt.savefig('./plots/indels.0.pdf')
	plt.close()
...
if plot_substitutions:
	...
	plt.title('Sal_R')
	plt.savefig('./plots/substitutions.0.png')
	if pdf_plots: plt.savefig('./plots/substitutions.0.pdf')
	plt.close()



dat = []
with open('./plots/depth.0.dat', 'rb') as f:
	reader = csv.reader(f, 'tab')
	for row in reader:
...
if plot_dp_dist:
	...
	plt.title('Sal_R')
	plt.savefig('./plots/depth.0.png')
	if pdf_plots: plt.savefig('./plots/depth.0.pdf')
	plt.close()
